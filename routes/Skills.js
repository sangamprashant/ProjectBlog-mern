...
const PROJECTBLOGSKILLS = mongoose.model("PROJECTBLOGSKILLS");
const requireLoginAdmin = require("../middleware/requireLogin");

// Route to add project blog skills
router.post("/api/admin/add/skills", requireLoginAdmin, async (req, res) => {
  try {
    ...
    const projectBlogDetail = new PROJECTBLOGSKILLS({
      label,
      value,
    });
...
router.get("/api/get/skills", async (req, res) => {
  try {
    ...
    const descriptionItems = await PROJECTBLOGSKILLS.find();
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      await PROJECTBLOGSKILLS.findByIdAndRemove(id);
      res
        .status(200)
