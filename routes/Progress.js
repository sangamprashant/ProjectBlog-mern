...
const PROJECTBLOGPROGRESS = mongoose.model("PROJECTBLOGPROGRESS");
const requireLoginAdmin = require("../middleware/requireLogin");

// Route to add project blog details
router.post("/api/admin/add/progress", requireLoginAdmin, async (req, res) => {
  try {
    ...
    const projectBlogDetail = new PROJECTBLOGPROGRESS({
      label,
      value,
    });
...
router.get("/api/get/progress", async (req, res) => {
  try {
    ...
    const descriptionItems = await PROJECTBLOGPROGRESS.find();
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      await PROJECTBLOGPROGRESS.findByIdAndRemove(id);
      res
        .status(200)
